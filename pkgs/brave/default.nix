{
  originalBrave,
  fetchurl,
  stdenv,
  ...
}@args:

let
  version = "1.92.138";

  sources = {
    x86_64-linux = {
      asset = "brave-browser_1.92.138_amd64.deb";
      hash = "sha256-MIBY6YMdpkMMoC9eKGBQMFuKN8csWa9fLfWFzeLTCyU=";
    };

    aarch64-linux = {
      asset = "brave-browser_1.92.138_arm64.deb";
      hash = "sha256-6m9BDgwCAbRZRYHE5JnL8kyR3XuMJ8dqUvkEQAXl7AI=";
    };
  };

  sys = stdenv.hostPlatform.system;
  braveArgs = builtins.removeAttrs args [
    "originalBrave"
    "fetchurl"
    "stdenv"
  ];
in
(originalBrave.override braveArgs).overrideAttrs (old: {
  inherit version;

  src = fetchurl {
    url = "https://github.com/brave/brave-browser/releases/download/v${version}/${sources.${sys}.asset}";
    hash = sources.${sys}.hash;
  };
})
